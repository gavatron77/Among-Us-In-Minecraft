...
execute as @a[tag=!ghost,tag=report,scores={emergency_press=1..}] unless score Emergency_Button_CD Main_Cooldowns > @s zero if entity @a[tag=imposter,scores={rectrfail_toggle=0,o2_fail_toggle=0,lights_toggle=0,comms_toggle=0}] at @a run playsound minecraft:entity.ender_dragon.death record @a ~ ~ ~ 0.2
